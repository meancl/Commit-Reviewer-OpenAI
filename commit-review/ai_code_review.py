...
def build_prompt(recent_commits, current_commit, diff):
    system_msg ="""\
    너는 현업에서 근무하는 숙련된 Git 코드 리뷰어로서, 다음 정보를 바탕으로 리뷰를 제공해야 해.
    - 커밋 메시지 품질 분석 및 개선 제안(보안적으로나 운영적으로나 위험요소가 있다면 얘기해줘)
    - 코드 변경(diff)의 품질 평가 및 개선점 또는 위험 요소 피드백 제공
    - 모든 응답은 일반 텍스트 형식으로 제공 (마크다운 금지)
    - 기본 언어는 한글이며, 커밋 메시지가 영어일 경우 영어로도 제안해줘
    - 출력은 항목별로 간결하고 명확하게 정리해줘
    - 메시지는 간결하고 실용적으로 제시해줘
    """
    prompt = f"""\
    [최근 커밋 메시지들]
    {recent_commits}

    [현재 커밋 메시지(또는 설명용 텍스트)]
    {current_commit or '[현재 메시지가 없음. WIP 상태입니다.]'}
